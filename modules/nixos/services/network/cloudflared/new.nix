    ...
    options = {
      ...
      hostname = lib.mkOption {
        ...
        description = ''
          ...
          Required if `ingress` is defined as a list. 
          If `ingress` is defined as an attribute set, this is automatically set to the attribute key.
        '';
      ...
      service = lib.mkOption {
        type = with lib.types; nullOr str;
        default = null;
        description = "Service to pass the traffic.";
      ...
      path = lib.mkOption {
        type = with lib.types; nullOr str;
        default = null;
        description = "Path filter. If not specified, all paths will be matched.";
            ...
            options = {
              ...
              ingress = lib.mkOption {
                description = ''
                  Ingress rules.
                  Can be defined as:
                  1. An attribute set (Legacy): `{"hostname" = "service";}` or `{"hostname" = { service = "..."; };}`.
                  2. A list (Recommended): `[ { hostname = "..."; service = "..."; } ... ]`.
                     The list format allows explicit ordering of rules and multiple rules per hostname.
                '';
                type =
                  with lib.types;
                  oneOf [
                    (listOf (submodule ingressRule))
                    (attrsOf (either str (submodule ingressRule)))
                  ];
                default = { };
                example = [
                  {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = lib.mapAttrs' (
      ...
      let
        ...
        processedIngress =
          if builtins.isList tunnel.ingress then
            map filterConfig tunnel.ingress
          else
            lib.concatLists (
              lib.mapAttrsToList (
                key: val:
        ...
        fullConfig = filterConfig {
          ...
          ingress = processedIngress ++ [ { service = tunnel.default; } ];
        };
