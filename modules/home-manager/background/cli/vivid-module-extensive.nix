...
let
  ...
  # mod-config = lib.attrsets.overrideExisting config {
  mod-config = {
    home.shell = {
...
in
{
  ...
  options.programs.vivid = {
    ...
    enableSessionVariables = mkEnableOption "Setting LS_COLORS session variable." // {
      default = true;
      exmaple = false;
    };
    enableBashIntegration = mkBashIntegrationOption { config = mod-config; };
    enableZshIntegration = mkZshIntegrationOption { config = mod-config; };
    enableFishIntegration = mkFishIntegrationOption { config = mod-config; };
    ...
    saveAsJson = mkEnableOption "Save file contents as json instead of yaml" // {
      default = true;
      exmaple = false;
    };
  };
  ...
  config =
    ...
    in
    mkMerge [
      (lib.mkIf (cfg.enable || (cfg.enableSessionVariables && cfg.themes != { })) {
        home.sessionVariables.LS_COLORS = "$(cat ${
          pkgs.runCommand "ls-colors" { nativeBuildInputs = [ cfg.package ]; }
            "vivid generate ${lib.optionalString (cfg.colorMode != null) "-m ${cfg.colorMode}"} ${
              if cfg.activeTheme == null then
                ""
              else if builtins.isAttrs cfg.themes.${cfg.activeTheme} then
                yamlFormat.generate "${cfg.activeTheme}.yml" cfg.themes.${cfg.activeTheme}
              else
        ...
        // (lib.mapAttrs' (
          ...
          lib.nameValuePair "vivid/themes/${name}.yml" (
            if lib.isAttrs value then
              ...
              {
                source =
                  if cfg.saveAsJson then
                    pkgs.writeText "${name}.json" (builtins.toJSON value)
                  else
                    yamlFormat.generate "${name}.yml" value;
              }
            else
              { source = value; }
          )
