...
- `systems/<arch>/<host>/` — one directory per host; the directory name **is** the hostname (forced via `networking.hostName`). `systems/default.nix` scans arch dirs and builds `darwinSystem` or `nixosSystem` accordingly. A `facter.json` in the host dir is picked up automatically for hardware config, and an optional `nixpkgs.nix` supplies per-host nixpkgs `config` (see below).
